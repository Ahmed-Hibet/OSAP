...
def filter_survey_for_respondent(user):
    age = None
    if user.birth_date:
        age = (datetime.date.today() - user.birth_date).days
        age//=365
    ...
    survey = Survey.objects.filter(
            ...
            Q(requirements__gender=user.gender)
        ).filter(
            Q(requirements__occupations__isnull=True) |
            Q(requirements__occupations__id=user.occupation.id)
        ).filter(
            Q(requirements__education_levels__isnull=True) |
            Q(requirements__education_levels__id=user.education_level.id)
        ).filter(
            ~Q(pk__in=[history.survey.id for history in RespondentHistory.objects.filter(respondent=user)])
        )
...
class SurveyCreate(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return filter_survey_for_respondent(self.request.user)  
